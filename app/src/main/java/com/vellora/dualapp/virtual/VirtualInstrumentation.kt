    ...
    override fun newActivity(cl: ClassLoader, className: String, intent: Intent?): Activity {
        ...
        if (targetPackage != null && targetClass != null) {
            ...
            if (targetLoader != null) {
                try {
                    // IMPORTANT ORDERING FIX: create the target's fake
                    // Application HERE, before the Activity object is even
                    // returned to ActivityThread — NOT later in
                    // callActivityOnCreate(). Android calls
                    // activity.attach() (which runs the target's own
                    // attachBaseContext() override) immediately after this
                    // function returns, BEFORE callActivityOnCreate() ever
                    // runs. Apps with custom SDK init in attachBaseContext()
                    // (confirmed case: Alibaba's AlibabaBaseLibActivity)
                    // crash there if they reach for Application-level
                    // singleton state that doesn't exist yet — moving this
                    // earlier gives that state a chance to already be ready.
                    VirtualPackageManager.applicationFor(appContext, targetPackage)
